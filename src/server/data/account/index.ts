...
export async function account_get(userPost: AccountType, users?: Repository<UsersRecord>): Promise<AccountState> {
  ...
  try {
    ...
    if (user) {
      ...
      return { account: {}, error: { name: "400", message: "account_get invalid user", stack: JSON.stringify(userPost) } };
    }
  } catch (e) {
    return { account: {}, error: { name: "400", message: "account_get catch", stack: JSON.stringify({ e, userPost }) } };
  }
}
...
export async function account_get_or_add(userPost: AccountType): Promise<AccountState> {
  ...
  try {
    ...
    return { account: {}, error: { name: "400", message: "account_get_or_add get catch", stack: JSON.stringify({ e, userPost }) } };
  }
  try {
    // add
    ...
    if (isAccountValid(account)) {
      ...
      return { account: {}, error: { name: "400", message: "account_get_or_add invalid user", stack: JSON.stringify(userPost) } };
    }
  } catch (e) {
    return { account: {}, error: { name: "400", message: "account_get_or_add add catch", stack: JSON.stringify({ e, userPost }) } };
  }
}
...
export async function account_update(userPost: AccountType): Promise<AccountState> {
  ...
  if (!userId) {
    return { account: {}, error: { name: "400", message: "account_update !userId", stack: JSON.stringify(userPost) } };
  }
  const user = await users.update(userId, userPost);
  if (isAccountValid(user)) {
    return { account: user };
  } else {
    return { account: {}, error: { name: "400", message: "account_update invalid user", stack: JSON.stringify(userPost) } };
  }
}
